...
public class ProjetDmpl implements ProjetDAO {
    ...
    public List findAll() {
        ...
        try {
            ...
            while (rs.next()){
                ...
                p.setDate(rs.getDate("dateLivraison"));
    ...
    public Projet save(Projet p) {
        try {
            Connection connection=new SingletonConnexionDB().getConnexion();
            PreparedStatement pstm=connection.prepareStatement("insert into projet (TITLE,dateLivraison ,priorité,RESPONSABLE_ID) values (?,?,?,?)");
            pstm.setString(1,p.getTITLE());
            pstm.setDate(2, (Date) p.getDate());
            pstm.setString(3, "high");
            pstm.setInt(4, 16);
            pstm.executeUpdate();
        }catch (SQLException e){
            e.printStackTrace();
