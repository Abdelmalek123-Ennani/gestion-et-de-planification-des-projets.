...
public class TaskDmpl implements TaskDAO {
    ...
    @Override
    public Materiel affecter_materiel(long taskId,long matId){
        try {
            Connection connection=new SingletonConnexionDB().getConnexion();
            PreparedStatement pstm=connection.prepareStatement("insert into affecter_materiel(TASK_ID , MATERIEL_ID ) values (?,?)");
            pstm.setLong(1,taskId);
            pstm.setLong(2,matId);
            pstm.executeUpdate();
        }catch (SQLException e){
            e.printStackTrace();
        }
        Materiel materielTr =  new Materiel();
        materielTr.setID(matId);
        return  materielTr;
    }
    ...
    public Intervenant affecter_intervenant(long t,long i){
        try {
            Connection connection=new SingletonConnexionDB().getConnexion();
            PreparedStatement pstm=connection.prepareStatement("insert into affecter_intervenant(TASK_ID, INTERVENANT_ID) values (?,?)");
            pstm.setLong(1,t);
            pstm.setLong(2,i);
            pstm.executeUpdate();
        }catch (SQLException e){
